...
rows = int(input())
# and the second row is still:
print(sum(matrix[row][row] for row in range(len(matrix))))
# but we are swaping the matrix with the other comprehension:
[[int(x) for x in input().split()] for _ in range(int(input()))]
# and for the range we are putting the "rows" again:
print(sum([[int(x) for x in input().split()] for _ in range(rows))][row][row] for row in range(rows)))
# ofcourse this isn't working and it is an awful solution and i don't want to fix it 
